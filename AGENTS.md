...
`coding-quota` is a Rust 2021 quota viewer for Codex, Grok, GLM, Kimi and Cursor. It provides a Windows-oriented terminal dashboard and an egui desktop widget; both consume the same credential and usage model.
...
- `fetch_all` uses one reqwest/rustls client with a 20-second timeout and `tokio::join!` for five providers. Provider parsers normalize usage into `used_fraction`; display remaining quota as `1 - used_fraction`.
