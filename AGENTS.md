...
`coding-quota` is a Rust 2021 quota viewer for Codex, Grok, GLM, Kimi, Cursor and Devin. It provides a Windows-oriented terminal dashboard and an egui desktop widget; both consume the same credential and usage model.
...
- `fetch_all` uses one reqwest/rustls client with a 20-second timeout and `tokio::join!` for six providers, and takes a skip list of tray-hidden providers (treated as unsubscribed: no request, no token refresh). Devin has no public quota API: the weekly quota is scraped live from the CLI banner via a hidden `conhost.exe --headless` launch of `devin.exe`, and the daily quota is read from the CLI's `user_status.*.bin` cache; only self-spawned PID trees may be terminated, never a Devin process by name. Provider parsers normalize usage into `used_fraction`; display remaining quota as `1 - used_fraction`.
